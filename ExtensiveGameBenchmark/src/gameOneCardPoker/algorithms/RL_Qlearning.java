...
public class RL_Qlearning extends Algorithm {
	...
	public RL_Qlearning( double alpha, double gamma, double epsilon )
	{
		super();
	...
	public void onlinePlay_GameOver( GameState gameState, long T )
	{
		...
		while( !gameStates.isEmpty() ) {
			 ...
			 * Q-learning
			 */
			double maxQ_Ip = Qs[seat][playerRank][nextHisID][0];
			for( int a = 1; a < Action.NUM_ACTION_TYPES; a++ ) {
				
				if( Qs[seat][playerRank][nextHisID][a] > maxQ_Ip )
					maxQ_Ip = Qs[seat][playerRank][nextHisID][a];
			}
			Q_Ia = ( 1 - ALPHA ) * Q_Ia + ALPHA * ( reward + GAMMA * maxQ_Ip );
