...
public class NashQ extends MARL
{
    ...
    */
	...
	for( int i = 0; i < xList.size(); i++ ) {
	     ...
	     */
	    Support[] supportProfile = new Support[GamblingGame.NUM_AGENTS];
	    for( int agentIndex = 0; agentIndex < GamblingGame.NUM_AGENTS; agentIndex++ ) {
	...
	if( index == GamblingGame.NUM_AGENTS ) {
		...
		for( int i = index+1; i < GamblingGame.NUM_AGENTS; i++ ) {
	...
	if( domPro == null || gameState == null ||
		domPro.size() != GamblingGame.NUM_AGENTS ) {
	...
	while( changed ) {
	     ...
	     */
	    for( int agent_i = 0; agent_i < GamblingGame.NUM_AGENTS; agent_i++ ) {
		...
		for( int a_i = 0; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
			 ...
			 */
			Support[] otherCups = new Support[GamblingGame.NUM_AGENTS];
			for( int j = 0; j < GamblingGame.NUM_AGENTS; j++ ) {
    ...
    {
	if( suppProfile == null || 
		suppProfile.length != GamblingGame.NUM_AGENTS ) {
	...
	try {
	   ...
	   //nashCP.setParameter(IloCP.ParameterValues.,2);
	     ...
	     */
	    int varNum = GamblingGame.NUM_AGENTS * GameAction.NUM_ACTIONS;
		...
		for( int a_i = 0; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
			...
			for( int listIndex = 0; listIndex < othJntActList_a.size(); listIndex++ ) {
			    ...
			    for( int agent_j = 0; agent_j < GamblingGame.NUM_AGENTS; agent_j++ ) {
    ...
    */
	 ...
	 */
	int minSumX = GamblingGame.NUM_AGENTS;
	int maxSumX = GameAction.NUM_ACTIONS * GamblingGame.NUM_AGENTS;
    ...
    private PriorityQueue<XVector> generateXVectors( int sum ) 
    {
    	 ...
    	 */
    	int agentNum = GamblingGame.NUM_AGENTS;
