...
import linearGridWorld.StateActionPair;
...
public class NashQ extends MARL
{
    ...
    */
	...
	for( int i = 0; i < xList.size(); i++ ) {
		...
		domainProfile.add( generateSupportDomain(xVector.getX(agentIndex)) );
	    }
	    
	    /**
	     * init the support profile to null
	     */
	    Support[] supportProfile = new Support[LinearGridWorld.NUM_AGENTS];
	    for( int agentIndex = 0; agentIndex < LinearGridWorld.NUM_AGENTS; agentIndex++ ) {
		
		supportProfile[agentIndex] = null;
	    }
	...
	if( index == LinearGridWorld.NUM_AGENTS ) {
		...
		for( int i = index+1; i < LinearGridWorld.NUM_AGENTS; i++ ) {
	...
	if( domPro == null || gameState == null ||
		domPro.size() != LinearGridWorld.NUM_AGENTS ) {
	...
	while( changed ) {
	     ...
	     */
	    for( int agent_i = 0; agent_i < LinearGridWorld.NUM_AGENTS; agent_i++ ) {
		...
		for( int a_i = 0; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
			 ...
			 */
			Support[] otherCups = new Support[LinearGridWorld.NUM_AGENTS];
			for( int j = 0; j < LinearGridWorld.NUM_AGENTS; j++ ) {
    ...
    {
	if( suppProfile == null || 
		suppProfile.length != LinearGridWorld.NUM_AGENTS ) {
	...
	try {
	   ...
	   // nashCP.setParameter(IloCP.ParameterValues.,2);
	     ...
	     */
	    int varNum = LinearGridWorld.NUM_AGENTS * GameAction.NUM_ACTIONS;
	    ...
	    IloNumVar[] p = nashCP.numVarArray( varNum, lowBounds, uppBounds);
		...
		for( int a_i = 0; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
			...
			for( int listIndex = 0; listIndex < othJntActList_a.size(); listIndex++ ) {
			    ...
			    for( int agent_j = 0; agent_j < LinearGridWorld.NUM_AGENTS; agent_j++ ) {
		...
		for( int a_i = firstSuppAction+1; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
    ...
    */
	 ...
	 */
	int minSumX = LinearGridWorld.NUM_AGENTS;
	int maxSumX = GameAction.NUM_ACTIONS * LinearGridWorld.NUM_AGENTS;
     ...
     */
    private ArrayList<GameAction> generateOtherJntActions( int agent_i, Support[] suppProfile )
    {
