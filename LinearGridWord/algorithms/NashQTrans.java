...
public class NashQTrans extends NashQ
{
    ...
    */
	...
	for( int stateIndex = 0; stateIndex < allStates.size(); stateIndex++ ) {
	    ...
	    //init equilibrium
	    double[] equil = new double[LinearGridWorld.NUM_AGENTS * actionNum];    
	    for( int act = 0; act < LinearGridWorld.NUM_AGENTS * actionNum; act++ ) {
		...
		double[] payoffs = new double[LinearGridWorld.NUM_AGENTS];
	...
	*/
	...
	if( b == null ) {
	    ...
	    System.out.println(gameState.getLocationID(0)+" "+
	    gameState.getLocationID(1)+" "+gameState.getLocationID(2));
	}
	...
	if( !visited.get( gameState ) ) {
	    ...
	    double[] retValues = new double[LinearGridWorld.NUM_AGENTS]; 
	    int actionNum = GameAction.NUM_ACTIONS;
	    
	    ArrayList<GameAction> allActions = LinearGridWorld.allJointActions;
	    
	    double[] deltaWeightedSums = new double[LinearGridWorld.NUM_AGENTS];
	    for( int agent = 0; agent < LinearGridWorld.NUM_AGENTS; agent++ ) 
		...
		for( int agent = 0; agent < LinearGridWorld.NUM_AGENTS; agent++ ) {
		    ...
		    for( int agent_p = 0; agent_p < LinearGridWorld.NUM_AGENTS; agent_p++ ) {
