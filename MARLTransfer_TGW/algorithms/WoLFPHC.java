...
public class WoLFPHC extends MARL
{
	...
	Q = new double[locNum][locNum][actionNum];
	pi = new double[locNum][locNum][actionNum];
	pi_bar = new double[locNum][locNum][actionNum];
	stateCounts = new int[locNum][locNum];
	for( int s1 = 0; s1 < locNum; s1++ )
	    for( int s2 = 0; s2 < locNum; s2++ ) {
			...
			pi_bar[s1][s2][a] = 1.0 / GameAction.NUM_ACTIONS;
		    }
		}
    }
