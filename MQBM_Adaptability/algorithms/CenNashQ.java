...
public class CenNashQ extends MARL
{
    ...
    public CenNashQ( double alpha, double gamma, double epsilon )
    {
    ...
    private double[] computeNE( int agent, GameState gameState )
    {
	if( gameState == null ) {
    ...
    //check
    private double[] computeNE_2agent( int agent, GameState gameState )
    {
	if( gameState == null ) {
    ...
    private boolean subSupport( Support supp1, Support supp2 )
    {
	//parameters
     ...
     */
    private double[] getNashQValues( GameState gameState, double[] nashE )
    {
    ...
    private double[] recursiveBackTracking( Support[] suppProfile, 
	...
	if( index == GridWorld.NUM_AGENTS ) {
		 ...
		 */
		ArrayList<ArrayList<Support>> domPro = new ArrayList<ArrayList<Support>>();
		...
		if( IRSDS( domPro, gameState ) ) {
		    ...
		    double[] nashEquil = recursiveBackTracking( suppProfile, domProfile, index+1, gameState );
	...
	while( changed ) {
		...
		for( int a_i = 0; a_i < GameAction.NUM_ACTIONS; a_i++ ) {
			...
			for( int a1 = 0; a1 < GameAction.NUM_ACTIONS; a1++ ) 
				...
				int[] jA_a = new int[]{a1,a1,a2};
				int[] jA_ap = new int[]{a1,a1,a2};
				jA_a[agent_i] = a_i;
				jA_ap[agent_i] = ap_i;
	...
	try {
	    ...
	    IloNumVar[] p = nashLCP.numVarArray(GridWorld.NUM_AGENTS*GameAction.NUM_ACTIONS, lowBounds, uppBounds);
	...
	*/
    ...
    private PriorityQueue<XVector> generateXVectors_2agent( int diff ) 
    {
	/**
    ...
    private ArrayList<Support> generateSupportDomain( int supportSize )
    {
     ...
     */
    private GameAction getJointAction_NE( double[] nashE )
    {	
