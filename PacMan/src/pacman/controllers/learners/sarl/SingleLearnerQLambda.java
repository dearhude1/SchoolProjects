...
public class SingleLearnerQLambda extends QLambda {
	...
	public SingleLearnerQLambda( Game game, GHOST learningGhost, 
			double lambda, boolean bLearn )
	{
		...
		valueFunctionFileName = 
				"./valueFunction_SingleLearnerQLambda_"+rlGhost.toString()+"_";
		if( !doesLearn )
			...
			double lambda, boolean bLearn )
	{
		 ...
		 * read the value function
		 */
		valueFunctionFileName = "./valueFunction_SingleLearnerQLambda_"+rlGhost.toString()+"_";
		if( !doesLearn )
			readValueFunction( valueFunctionFileName );
	...
	protected void updateValueFunction_EpiEnded( Game curGame )
	{
		...
		if( ghostLastMov != MOVE.NEUTRAL ) {
			...
			double ghostLastMaxQvalue = getMaxQValue(lastGame, rlGhost);
			double ghostLastQvalue = getQValue(lastGame, rlGhost, ghostLastStrategy);
			...
			for( int strategy = 0; strategy < Constants.NUM_GHOST_STRATEGIES_SINGLE_AGENT; strategy++ ) {
				...
				if( Math.abs(ghostLastMaxQvalue - ghostLastQvalue) < 0.00001 ) {
					...
					for( int fIndex = 0; fIndex < Constants.NUM_FEATURES_SINGLE_AGENT; fIndex++ ) {
						...
						ghostEligMap.get(rlGhost)[strategy][fIndex] = 
								ghostEligMap.get(rlGhost)[strategy][fIndex] * GAMMA * LAMBDA + 
					...
					for( int fIndex = 0; fIndex < Constants.NUM_FEATURES_SINGLE_AGENT; fIndex++ ) 
						ghostEligMap.get(rlGhost)[strategy][fIndex] = 0.0;
				}
			}
			/////////////////////////////////////////////
			...
			for( int strategy = 0; strategy < Constants.NUM_GHOST_STRATEGIES_SINGLE_AGENT; strategy++ ) {
				for( int fIndex = 0; fIndex < Constants.NUM_FEATURES_SINGLE_AGENT; fIndex++ ) {
					...
					ghostThetaMap.get(rlGhost)[strategy][fIndex] += 
							ALPHA * delta * ghostEligMap.get(rlGhost)[strategy][fIndex];
				}
			}
