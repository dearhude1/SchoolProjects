...
import pacman.game.internal.MazeGraph;
...
public final class Game
{
	...
	//////////////////////////////////////////////////
	//-----------modified by dearhude1------------////
	//////////////////////////////////////////////////
	/**
	 * the ghost which eat the pacman in the last step
	 */
	public GHOST ghostEatsPacman;
	
	/**
	 * the game copy when the pacman is eaten 
	 * for recording the positions of pacman, ghosts 
	 * and their moves
	 */
	public Game gameCopyWhenPacmanEaten = null;
	
	/**
	 * the moving direction of the ghosts when they are eaten 
	 * such information should be recorded since the ghosts 
	 * will be reset right after they are eaten
	 */
	public EnumMap<GHOST, MOVE> ghostsMovWhenEaten = 
			new EnumMap<Constants.GHOST, Constants.MOVE>(GHOST.class);
	/**
	 * the position of the ghosts when they are eaten
	 */
	public EnumMap<GHOST, Integer> ghostsPositionWhenEaten = 
			new EnumMap<Constants.GHOST, Integer>(GHOST.class);
	
	/**
	 * the node index of the power pill eaten the most recently
	 */
	public int nodeIndexPowerPillEaten = -1;
	
	//////////////////////////////////////////////////
	///////////////Added by dearhude1/////////////////
	//////////////////////////////////////////////////
	/**
	 * the maze graph of the current maze
	 */
	...
	public static final int EPISODE_NUM = 1000;//200000;//800000;//10000000;
	
	/**
	 * the time step after the power pills were eaten
	 *
	private int[] powerPillEatenTime = null;
	private int powerPillRecoverTime = 300;
	*/
	
	/**
	 * the score gained by pacman in an episode
	 */
	private double lostScoreOneEp = 0;
	
	/**
	 * the remaining score after an episode ends
	 */
	private double remainingScoreOneEp = 0;

	/**
	 * was the last episode ended
	 */
	private boolean wasEpiEnded = false;
	
	/**
	 * three important variables when an episode ends
	 */
	private double lostScoreWhenEpEnded = 0;
	private double remainingScoreWhenEpEnded = 0;
	private int levelTimeWhenEpEnded = 0;
	
	/**
	 * for computing the additional score 
	 * for the ghosts not being eaten during a power pill time
	 */
	private int[] numGhostsEatenForEachPowerPill;
	private int eatenPowerPillIndex = -1;
	private double scoreForGhostsNotEaten = 0;
	
	
	
	//private double[] scorePerEP = new double[EPISODE_NUM]; 
	...
	{		
		...
		//////////////////////////////
		/**
		 * create the current maze graph
		 */
		mazeGraph = new MazeGraph( this );
		
		/**
		 * create the array of power pill eaten time
		 *
		powerPillEatenTime = new int[getPowerPillIndices().length];
		for( int powerPillIndex = 0; powerPillIndex < powerPillEatenTime.length; powerPillIndex++ ) {
			
			powerPillEatenTime[powerPillIndex] = -1;
		}
		*/
		
		/**
		for( int ep = 0; ep < EPISODE_NUM; ep++ ) {
	...
	private void _init(int initialMaze)
	{
		...
		/////////////////////////////////////////
		lostScoreOneEp = 0;
		remainingScoreOneEp = 0;
		wasEpiEnded = false;
		
		eatenPowerPillIndex = -1;
		numGhostsEatenForEachPowerPill = new int[currentMaze.powerPillIndices.length];
		for( int i = 0; i < numGhostsEatenForEachPowerPill.length; i++ )
			numGhostsEatenForEachPowerPill[i] = 0; 
		scoreForGhostsNotEaten = 0;
	}
	...
	///////////////////////////////////
	/**
	 * added for RL algorithms
	 * this method reset the level as follows:
	 * reset the level pills and power pills
	 * reset the lives of Pacman
	 * reset the locations of Pacman and Ghosts
	 */
	private void _newEpisode()
	{		
		_initGhosts();
		
		pacman.currentNodeIndex = currentMaze.initialPacManNodeIndex;
		pacman.lastMoveMade = MOVE.LEFT;
		pacman.numberOfLivesRemaining = NUM_LIVES;
		
		/**
		 * set the pills and level time
		 */		
		currentLevelTime = 0;
		ghostEatMultiplier = 1;	
		_setPills(currentMaze);
		...
		/**
		 * set the number of eaten ghosts for each power pill
		 */
		eatenPowerPillIndex = -1;
		for( int i = 0; i < numGhostsEatenForEachPowerPill.length; i++ )
			numGhostsEatenForEachPowerPill[i] = 0; 
		 ...
		 */
		System.out.println("Current Episode: "+episode+"=================");
		
		/**
		 * record the score obtained so far
		 *
		if( episode < EPISODE_NUM )
			scorePerEP[episode] = score;
		*/
	...
	public Game copy()
	{
		...
		////////////////////////////////////
		/**
		 * also copy the ghost which eats Pacman
		 */
		copy.ghostEatsPacman = ghostEatsPacman;
		/**
		 * also copy the maze graph
		 */
		copy.mazeGraph = mazeGraph;
		/**
		 * also copy the power pill eaten time
		 *
		copy.powerPillEatenTime = powerPillEatenTime;
		copy.powerPillRecoverTime = powerPillRecoverTime;
		*/
		
		/**
		 * also copy the current episode 
		 * and the last episode
		 */
		copy.episode = episode;
		copy.lastEpisode = lastEpisode;
		
		copy.gameCopyWhenPacmanEaten = gameCopyWhenPacmanEaten;
		copy.ghostsMovWhenEaten = ghostsMovWhenEaten;
		copy.ghostsPositionWhenEaten = ghostsPositionWhenEaten;
		copy.nodeIndexPowerPillEaten = nodeIndexPowerPillEaten;
		
		/**
		 * set the scores
		 */	
		copy.lostScoreOneEp = lastEpisode;
		copy.remainingScoreOneEp = remainingScoreOneEp;
		/**
		 * set the boolean variable wasEpiEnded
		 */
		copy.wasEpiEnded = wasEpiEnded;
		/**
		 * set three important variables
		 */
		copy.lostScoreWhenEpEnded = lostScoreWhenEpEnded;
		copy.remainingScoreWhenEpEnded = remainingScoreWhenEpEnded;
		copy.levelTimeWhenEpEnded = levelTimeWhenEpEnded;
		/**
		 * 
		 */
		copy.scoreForGhostsNotEaten = scoreForGhostsNotEaten;
		copy.eatenPowerPillIndex = eatenPowerPillIndex;
		copy.numGhostsEatenForEachPowerPill = numGhostsEatenForEachPowerPill;
		
		//copy.scorePerEP = scorePerEP;
	...
	public void updateGame()
	{
		...
		/////////////////////////////////////////
		/**
		 * do not increase the total time
		 */
		//totalTime++;
		
		/**
		 * check whether an episode ends
		 */
		_checkEpisodeState();
	...
	private void _eatPowerPill()
	{
		...
		if(powerPillIndex>=0 && powerPills.get(powerPillIndex))
		{
			 ...
			 * update the score
			 */
			lostScoreOneEp += POWER_PILL;
			
			/**
			 * update the eaten ghosts num
			 */
			eatenPowerPillIndex += 1;
			numGhostsEatenForEachPowerPill[eatenPowerPillIndex] = 0;
			
			//////////////////////////////////////////////////
			//-----------modified by dearhude1------------////
			//////////////////////////////////////////////////
			/**
			 * we record the position of the eaten power pill
			 */
			nodeIndexPowerPillEaten = pacman.currentNodeIndex;
		}
		//no power pill is eaten
		else
			nodeIndexPowerPillEaten = -1;
	}
		...
		for(Ghost ghost : ghosts.values())
		{
			...
			if(distance<=EAT_DISTANCE && distance!=-1)
			{
				if(ghost.edibleTime>0)									//pac-man eats ghost
				{
					...
					//////////////////////////////////////////////////
					/**
					 * we should record the information 
					 * of the eaten ghost before it is reset
					 */
					if( ghostsMovWhenEaten.containsKey(ghost.type) )
						ghostsMovWhenEaten.remove(ghost.type);
					ghostsMovWhenEaten.put(ghost.type, ghost.lastMoveMade);
					if( ghostsPositionWhenEaten.containsKey(ghost.type) )
						ghostsPositionWhenEaten.remove(ghost.type);
					ghostsPositionWhenEaten.put(ghost.type, ghost.currentNodeIndex);
					...
					/**
					 * update the score 
					 * note that the ghostEatMultiplier has been already doubled
					 * so we need to divide the incremental value by 2
					 */
					lostScoreOneEp += GHOST_EAT_SCORE * ghostEatMultiplier / 2;
					
					/**
					 * update the eaten ghosts number
					 */
					numGhostsEatenForEachPowerPill[eatenPowerPillIndex] += 1;
				}
				else													//ghost eats pac-man
				{
					...
					//////////////////////////////////////////////////
					/**
					 * record which ghost eat the pacman in this step
					 */
					ghostEatsPacman = ghost.type;
					/**
					 * record the position where pacman is eaten 
					 * before the level is reset 
					 * also record the node indices of the ghosts and their moves
					 * we can just copy the current game before level is reset
					 */
					gameCopyWhenPacmanEaten = null;
					gameCopyWhenPacmanEaten = copy();
					...
					if(pacman.numberOfLivesRemaining<=0) {
						...
						//new episode, not game over
						//do not call newEpisode here
						//_newEpisode();
						System.out.println("Pacman 0 life, new episode will begins");
					}
					else {
	 ...
	 * check whether an episode ends
	 */
	private void _checkEpisodeState()
	{
		wasEpiEnded = false;
		
		/**
		 * if all pills have been eaten or the time is up 
		 * or the lives of pacman becomes 0, 
		 * then an episode ends
		 */
		if( (pills.isEmpty() && powerPills.isEmpty()) || 
				currentLevelTime >= LEVEL_LIMIT || 
				pacman.numberOfLivesRemaining <= 0 ) {
			
			wasEpiEnded = true;
			
			/**
			 * record the information for learning
			 * compute the remaining score
			 */
			_computeRemainingScore();
			
			/**
			 * compute the additional score for eaten ghosts
			 */
			_computeScoreForEatenGhosts();
			
			/**
			lostScoreOneEp += pacman.numberOfLivesRemaining * AWARD_LIFE_LEFT;
			remainingScoreOneEp += (NUM_LIVES-pacman.numberOfLivesRemaining) * AWARD_LIFE_LEFT;
			remainingScoreOneEp += scoreForGhostsNotEaten;
			lostScoreWhenEpEnded = lostScoreOneEp;
			remainingScoreWhenEpEnded = remainingScoreOneEp;
			levelTimeWhenEpEnded = currentLevelTime;
			*/
	...
	/**
	 * check whether the game (or rather training) is over
	 */
	private void _checkGameOver()
	{
		/**
		 * if the episode reach the limit 
	...
	/**
	 * compute the remaining score after an episode ends
	 */
	private void _computeRemainingScore()
	{
		remainingScoreOneEp = 0;
		...
		for( int i = 0; i < currentMaze.pillIndices.length; i++ ) {
			if( isPillStillAvailable(i) )
				remainingScoreOneEp += PILL;
		}
		
		/**
		 * the remaining power pills
		 */
		for( int i = 0; i < currentMaze.powerPillIndices.length; i++ ) {
			if( isPowerPillStillAvailable(i) )
				remainingScoreOneEp += POWER_PILL;
		}
		
		//System.out.println("Pill "+remainingScoreOneEp);
	}
	
	
	private void _computeScoreForEatenGhosts()
	{
		scoreForGhostsNotEaten = 0;
		
		for( int ppIndex = 0; ppIndex < numGhostsEatenForEachPowerPill.length; ppIndex++ ) {
			
			int eatenNum = numGhostsEatenForEachPowerPill[ppIndex];
			int remainNum = currentMaze.powerPillIndices.length - eatenNum;
			...
			for( int j = 0; j < remainNum; j++ ) {
				...
				score += GHOST_EAT_SCORE * multiplier;
				multiplier *= 2;
			}
			scoreForGhostsNotEaten += score;
		}
	}
	...
	public boolean doesGhostRequireAction(GHOST ghostType)
	{
		...
		*/
		...
		//////////////////////////////////////////////////
		/**
		 * note that a junction is a node that connects with at least 3 other nodes
		 * but in a node with fewer than 2 neighbour nodes, the ghosts can always make a decision.
		 * Therefore, I remove the condition of isJunction(...)
		 * 
		 * In our settings, ghosts can only take an action in a Non-Pipe Node
		 */
		if( ghosts.get(ghostType).edibleTime == 0 || 
				ghosts.get(ghostType).edibleTime%GHOST_SPEED_REDUCTION != 0 ) {
			
			if( ghosts.get(ghostType).lastMoveMade == MOVE.NEUTRAL ) {
				
				if( ghosts.get(ghostType).currentNodeIndex == currentMaze.initialGhostNodeIndex ) {
					
					//System.out.println("Require actions when intial");
					return true;
				}
				else
					return false;
				...
				int ghostNodeIndex = ghosts.get(ghostType).currentNodeIndex;
				if( !isJunction( ghostNodeIndex ) ) 
					return false;
				else
					return true;
			}
		}
		//if the ghost now is edible or cannot move in edible time
		else 
			return false;
	...
	//-----------added by dearhude1------------///////
	//////////////////////////////////////////////////
	public boolean doesPacmanRequireAction()
	{
		return ( !isJunction(pacman.currentNodeIndex) || 
				( pacman.lastMoveMade == MOVE.NEUTRAL && 
				pacman.currentNodeIndex == currentMaze.initialPacManNodeIndex ) );
	}
	...
	//////////////////////////////////////////////////
	/**
	 * check whether a node is in a pipe, 
	 * that is, the node has exact to possible move directions 
	 * and the two directions are opposite
	 */
	public boolean isPipe( int nodeIndex ) 
	{		
		if( nodeIndex == currentMaze.initialGhostNodeIndex || 
				nodeIndex == currentMaze.initialPacManNodeIndex || 
				nodeIndex == currentMaze.lairNodeIndex )
			return true;		
		else if( currentMaze.graph[nodeIndex].numNeighbouringNodes != 2 ) {
			...
			for( MOVE move : MOVE.values() ) {
				...
				Node curNode = currentMaze.graph[nodeIndex];
				if( curNode.neighbourhood.containsKey( move ) && 
					curNode.neighbourhood.containsKey( move.opposite() ) ) {
    ...
    */
	...
	public MOVE getNextMoveTowardsTarget(int fromNodeIndex,int toNodeIndex,MOVE lastMoveMade, DM distanceMeasure)
	{
		...
		////////////////////////////////////////////
		/**
		 * encourage to approach in various directions
		 *
		ArrayList<MOVE> possibleMoves = new ArrayList<Constants.MOVE>();
		for(Entry<MOVE,Integer> entry : currentMaze.graph[fromNodeIndex].allNeighbourhoods.get(lastMoveMade).entrySet())
		{
			double distance = getDistance(entry.getValue(), toNodeIndex, 
					lastMoveMade, distanceMeasure);
			...
			{
				MOVE possMove = entry.getKey();
				possibleMoves.add( possMove );
			}
		}
		
		int size = possibleMoves.size();
		return possibleMoves.get( rnd.nextInt(size) );
		*/
	...
	public MOVE getNextMoveAwayFromTarget(int fromNodeIndex,int toNodeIndex,MOVE lastMoveMade, DM distanceMeasure)
	{
		...
		////////////////////////////////////////////
		/**
		 * encourage to run away in various directions
		 *
		ArrayList<MOVE> possibleMoves = new ArrayList<Constants.MOVE>();
		for(Entry<MOVE,Integer> entry : currentMaze.graph[fromNodeIndex].allNeighbourhoods.get(lastMoveMade).entrySet())
		{
	...
	/////////////////////Added by dearhude1///////////////
	//////////////////////////////////////////////////////
	public MazeGraph getCurrentMazeGraph()
	{
		return mazeGraph;
	}
	...
	/**
	 * return the maximal node distance in the current maze
	 */
	public int getCurrentMaxNodeDis()
	{
		return currentMaze.maxNodeDis;
	}
	
	/**
	 * find the index of the next NPP node to be reached 
	 * given the current position and the moving direction
	 */
	public int getNextNPPNodeIndex( int nodeIndex, MOVE direction )
	{
		if( !isPipe(nodeIndex) ) {
			
			System.out.println("getNextNPPNodeIndex: It is an NPP!");
			return nodeIndex;
		}
		
		int neighborNodeIndex = getNeighbour( nodeIndex, direction );
		
		if( neighborNodeIndex == -1 ) {
			
			
			System.out.println("-1 Neibor Index, Node Index "+nodeIndex+" Move "+direction.toString()+
					" initial "+getGhostInitialNodeIndex());
			
			return nodeIndex;
		}
		
		while( isPipe(neighborNodeIndex) ) {
			...
			neighborNodeIndex = getNeighbour( neighborNodeIndex, direction );
	...
	public boolean wasEpisodeEnded()
	{
		return wasEpiEnded;
	}
	
	/**
	 * get the lost score after an episode ends
	 */
	public double getLostScoreWhenEpEnded()
	{
		return lostScoreWhenEpEnded;
	}
	
	/**
	 * get the remaining score after an episode ends
	 */
	public double getRemainingScoreWhenEpEnded()
	{
		return remainingScoreWhenEpEnded;
	}
	
	/**
	 * get the level time used after an episode ends
	 */
	public int getLevelTimeWhenEpEnded()
	{
		return levelTimeWhenEpEnded;
	}
	
	public double getRemainingPillRate()
	{
		double count = 0.0;
		 ...
		 * the remaining pills
		 */
		int allPillNum = currentMaze.pillIndices.length;
		for( int i = 0; i < allPillNum; i++ ) {
			if( isPillStillAvailable(i) )
				count += 1.0;
		}
		
		return count / allPillNum; 
	}
	
	public double getRemainingPillScore()
	{
		...
		return count * Constants.PILL;
	}
	
	public double getRemainingPowerPillRate()
	{
		double count = 0.0;
		 ...
		 * the remaining power pills
		 */
		int allPowerPillNum = currentMaze.powerPillIndices.length;
		for( int i = 0; i < currentMaze.powerPillIndices.length; i++ ) {
			if( isPowerPillStillAvailable(i) )
				count += 1.0;
		}
		
		return count / allPowerPillNum;
	}
	
	public double getRemainingPowerPillScore()
	{
		double count = 0.0;
		 ...
		 */
		int allPowerPillNum = currentMaze.powerPillIndices.length;
		for( int i = 0; i < allPowerPillNum; i++ ) {
			if( isPowerPillStillAvailable(i) )
				count += 1.0;
		}
		
		return count * Constants.POWER_PILL;
	}
	
	public double getPacmanRemainingLifeRate()
	{
		return ((double)pacman.numberOfLivesRemaining) / ((double)NUM_LIVES);
	}
	
	public int getGhostEatMultiplier()
	{
		return ghostEatMultiplier;
	}
	
	public double[] getScorePerEP()
	{
		return null;
		//return scorePerEP;
	}
