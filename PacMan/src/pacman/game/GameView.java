...
import pacman.game.internal.MazeGraph.Connection;
...
{	
    ...
    public void paintComponent(Graphics g) 
    {
        ...
        /////////////////////////////////////
        /**
         * draw non-pipe nodes and all connections
         */
        drawNPPNode();
        drawConnections();
        drawPacmanConnection();
    	...
    	for(GHOST ghostType : GHOST.values())
    	{
	    	...
	    	if(game.getGhostEdibleTime(ghostType)>0)
	    	{
	    		...
	    		if(game.getGhostEdibleTime(ghostType)<EDIBLE_ALERT && ((time%6)/3)==0) {
	    			...
	    			//bufferGraphics.drawString(String.valueOf(currentNodeIndex), nodeXCood*MAG-1, nodeYCood*MAG+3);
	    		}
	            else {
	    	...
	    	else 
	    	{
	    		...
	    		if(game.getGhostLairTime(ghostType)>0) {
	    			...
	    			bufferGraphics.drawImage(images.getGhost(ghostType,game.getGhostLastMoveMade(ghostType),time), 
	    					nodeXCood*MAG-1+(index*5),nodeYCood*MAG+3,null);
	    			//bufferGraphics.drawString(String.valueOf(currentNodeIndex), nodeXCood*MAG-1+(index*5),nodeYCood*MAG+3);
	    		}
	    		else {   
	    			
	    			bufferGraphics.drawImage(images.getGhost(ghostType,game.getGhostLastMoveMade(ghostType),time), 
	    					nodeXCood*MAG-1,nodeYCood*MAG+3,null);
	    			//bufferGraphics.drawString(String.valueOf(currentNodeIndex), nodeXCood*MAG-1,nodeYCood*MAG+3);
	    		}
	    	}
    	}
    ...
    /////////////////////////////////////////////////////
    /**
     * draw the NonPipeNode in the map
     */
    private void drawNPPNode()
    {
    	int[] nppNodeIndices = game.getCurrentMazeGraph().getAllNPPNodeIndices();
    	int numNPP = nppNodeIndices.length;
    	
    	Color lastColor = bufferGraphics.getColor();
    	bufferGraphics.setColor( Color.GREEN );
    	for( int nppIndex = 0; nppIndex < numNPP; nppIndex++ ) {
    		
    		int nppNodeIndex = nppNodeIndices[nppIndex];
    		int nodeXCood = game.getNodeXCood( nppNodeIndex );
    		int nodeYCood = game.getNodeYCood( nppNodeIndex );
    		
    		bufferGraphics.drawRect(nodeXCood*MAG-3, nodeYCood*MAG-3, 6, 6);
    		
    	}
    	
    	/**
    	 * recove the last color
    	 */
    	bufferGraphics.setColor( lastColor );
    }
    ...
    {
    	Connection[] connections = game.getCurrentMazeGraph().getAllConnections();
    	int numConn = connections.length;
    	...
    	for( int connIndex = 0; connIndex < numConn; connIndex++ ) {
    		...
    		int[] nppNodeIndices = connection.getNPPNodeIndices();
    		int nppXCood_1 = game.getNodeXCood( nppNodeIndices[0] );
    		int nppYCood_1 = game.getNodeYCood( nppNodeIndices[0] );
    		int nppXCood_2 = game.getNodeXCood( nppNodeIndices[1] );
    		int nppYCood_2 = game.getNodeYCood( nppNodeIndices[1] );
    		
    		bufferGraphics.drawLine( nppXCood_1*MAG, nppYCood_1*MAG, 
    				nppXCood_2*MAG, nppYCood_2*MAG );
    	}
    ...
    private void drawPacmanConnection()
    {
    	
    	int pacmanNodeIndex = game.getPacmanCurrentNodeIndex();
    	MOVE pacmanMove = game.getPacmanLastMoveMade();
    	
    	int pamanConnIndex = game.getCurrentMazeGraph().queryConnectionIndex( game, 
    			pacmanNodeIndex, pacmanMove );
    	
    	Connection[] connections = game.getCurrentMazeGraph().getAllConnections();
    	

    	
		Connection connection = connections[pamanConnIndex];
		int[] nppNodeIndices = connection.getNPPNodeIndices();
		int nppXCood_1 = game.getNodeXCood( nppNodeIndices[0] );
		int nppYCood_1 = game.getNodeYCood( nppNodeIndices[0] );
		int nppXCood_2 = game.getNodeXCood( nppNodeIndices[1] );
