	...
	public Node(int nodeIndex,int x,int y,int pillIndex,int powerPillIndex,int[] _neighbourhood)
	{
		...
		for( int i = 0; i < moves.length; i++ ) {
			...
			if( neighbourhood.containsKey( moves[i] ) ) {
				...
				int neighIndex = neighbourhood.get( moves[i] );
				System.out.print(moves[i]+" "+neighIndex+ ",");
			}
		}
		System.out.println();
		for( int i = 0; i < moves.length; i++ ) {
			
			if( allPossibleMoves.containsKey( moves[i] )){
				
				MOVE[] ms = allPossibleMoves.get( moves[i] );
				System.out.println("Move "+moves[i]+" has "+ms.length+" possible moves-------");
