...
public class PCEQTransModel extends PCEQ
{
    ...
    public PCEQTransModel( double alpha, double gamma, double epsilon )
    {
	...
	transCndValues = new double[agentNum];
	transCndValues[0] = 13;//1.5;//1.5
	transCndValues[1] = 13;//3.0;//3.0;
	...
	if( nextState == null ) {
		...
		ALPHA *= 0.9991;//0.998;//0.998;//0.995;//0.99988;//58;//0.9975;//0.99958;
	...
	for( int stateIndex = 0; stateIndex < allStates.size(); stateIndex ++ ) {
		...
		if( stateSimilarity[agent][locState] < transCndValues[agent] ) {
			...
			setQValue( agent, state, jntAction, 200.0 );  
