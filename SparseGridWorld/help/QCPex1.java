...
public class QCPex1 {
   ...
   public static void main(String[] args) {
	   ...
	   */
         ...
         /**
          * IloRange is used for range constraints
          */
   ...
   static IloNumVar[] populateByRow(IloMPModeler model,
       ...
       * and restrict their low bounds and upper bounds
       */
      IloNumVar[] x  = model.numVarArray(3, lb, ub);

      
      /**
       * 2. then create several constraints
       */
      /**
       * there are three range constraints
       * therefore, the length of row is 3
       */
      // - x0 +   x1 + x2 <= 20
      //   x0 - 3*x1 + x2 <= 30
      // val stores coefficients in the range expressions
      double[][] val = { {-1.0,  1.0,  1.0},
                         { 1.0, -3.0,  1.0} };
      
      /**
       * scalProd: scale the variables
       */
