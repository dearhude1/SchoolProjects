...
public class FFQ extends MARL
{
    ...
    double alphaDecay = 0.999;//0.9999954;//0.99999;
	...
	for( int stateIndex = 0; stateIndex < allStates.size(); stateIndex++ ) {
	     ...
	     * for policy
	     */
	    int teamActionNum = GamblingGame.allTeamActions.size();
	    double[] policies = new double[teamActionNum];
	    for( int teamActionIndex = 0; teamActionIndex < teamActionNum; teamActionIndex++ ) {
		    
		    policies[teamActionIndex] = 1.0 / teamActionNum;
	    } 
    ...
    */
	...
	if( !pi.containsKey( gameState ) ) {
	    ...
	    System.out.println("Pot: "+gameState.getPot()+" BigSmall: "+gameState.getBigSmall());
	    ...
	    */
	     ...
	     */
	    int teamActionIndex = GamblingGame.queryTeamActionIndex(gameState, teamIndex, teamAction);
