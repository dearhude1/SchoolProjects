...
public class MARL
{
    ...
    public static final int TEAMQ = 11;
    public static final int JAL = 12;
    public static final int DISTRI_Q = 13;
    public static final int FRIENDQ = 14;
    public static final int RANDOM = 15;
    
    
    public static final String[] ALG_STRINGS = { "minimaxQ", "NashQ", "FFQ", "decCEQ", "cenCEQ", 
	"uCEQ", "eCEQ", "pCEQ", "dCEQ", "NegoQ", "WoLF-PHC", "teamQ", "JAL", "distributedQ", 
	"friendQ", "Random" };
    ...
    protected double EPSILON = 0.09;//1;//0.01;
	...
	Qs = new double[PursuitGame.NUM_AGENTS][locNum][locNum][locNum][actionNum][actionNum][actionNum];
	vstNum = new double[locNum][locNum][locNum][actionNum][actionNum][actionNum];
			...
			for( int a1 = 0; a1 < actionNum; a1++ )
			    for( int a2 = 0; a2 < actionNum; a2++ ) 
			    	for( int a3 = 0; a3 < actionNum; a3++ ) {
			    	    ...
			    	    Qs[agentIndex][s1][s2][s3][a1][a2][a3] = 0.0;
				    
			    	    vstNum[s1][s2][s3][a1][a2][a3] = 0.0;
			    	}
    }
	...
	jntAction.setAction( 2, random.nextInt(GameAction.NUM_ACTIONS) );
	...
	if( vstNum[loc0][loc1][loc2][a0][a1][a2] <= 0.0 )
	    return 1.0;
	else
	    return 1.0 / vstNum[loc0][loc1][loc2][a0][a1][a2];
	...
	return Qs[agent][loc0][loc1][loc2][a0][a1][a2];
    }
	...
	Qs[agent][loc0][loc1][loc2][a0][a1][a2] = value;
    }
			...
			for( int a1 = 0; a1 < actionNum; a1++ )
			    for( int a2 = 0; a2 < actionNum; a2++ ) 
			    	for( int a3 = 0; a3 < actionNum; a3++ ) {
			    	    ...
			    	    writer.write(""+Qs[agentIndex][s1][s2][s3][a1][a2][a3]);
			    	    writer.newLine();
			    	}
	    writer.close();
	}
	...
	try {
		...
		Qs[agent][s1][s2][s3][a1][a2][a3] = Double.parseDouble( line );
