...
public class DecenCEQTrans extends DecenCEQ
{
	...
	if( nextState == null ) {
		...
		correlEquil = lastEquilibrium[nextState.getWorldIndex()][nextState.getLocationID(0)][nextState.getLocationID(1)];
	...
	for( int jntAct = 0; jntAct < GameAction.NUM_ACTIONS*GameAction.NUM_ACTIONS; jntAct++ ) {
	    ...
	    proSum += lastEquilibrium[worldIndex][loc0][loc1][jntAct];
	}
	...
	if( !visited[worldIndex][loc0][loc1] ) {
	    ...
	    double[] retValues = new double[MultiGridWorld.NUM_AGENTS];
	...
	if( gameState == null || agent < 0 || 
		agent >= MultiGridWorld.NUM_AGENTS ) {
	...
	int agent_o = (agent+1) % MultiGridWorld.NUM_AGENTS;
	...
	for( int act = 0; act < GameAction.NUM_ACTIONS; act++ ) {
		...
		for( int act_o = 0; act_o < GameAction.NUM_ACTIONS; act_o++ ) {
		    ...
		    loss += lastEquilibrium[worldIndex][loc0][loc1][jntActIndex] * deltaUti;
		}
