...
public class MiniMaxQTrans extends MiniMaxQ
{
	...
	if( nextState == null ) {
		...
		if( bCompute ) {
		    ...
		    double minimaxV = getPolicyMinValue( curState, pi[loc0][loc1][ballPoss]);
		    setV( curState,  minimaxV );
		}
	 ...
	 * the last policy
	 */
	double[] lastPolicy = pi[loc0][loc1][ballPoss]; 
	...
	for( int act_o = 0; act_o < GameAction.NUM_ACTIONS; act_o++ ) {
		...
		jntAction.setAction( (agentIndex+1) % SoccerGame.NUM_AGENTS, act_o);
