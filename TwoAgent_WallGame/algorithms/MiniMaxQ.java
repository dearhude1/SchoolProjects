...
public class MiniMaxQ extends MARL
{
    ...
    double alphaDecay = 0.9999539;//0.99999;
	...
	for( int s1 = 0; s1 < locNum; s1++ )
		...
		boolean[] availActs = WallGame.actionSet(loc);
		int availNum = 0;
	...
	double proSum = probabilities[0] = getPi(gameState, 0);
	if( !actAvail[0] )
	    proSum = probabilities[0] = 0.0;
	for( int act = 1; act < GameAction.NUM_ACTIONS; act++ )
	{
	    
	    if( !actAvail[act] )
		probabilities[act] = 0.0;
	    else {
		
		proSum += getPi(gameState, act);
		probabilities[act] = proSum;
	    }
	}
    ...
    */
	...
	int oppIndex = (agentIndex+1) % WallGame.NUM_AGENTS;
	boolean[] actAvail_opp =  WallGame.actionSet( gameState.getLocationID(oppIndex) );
	boolean[] actAvail = WallGame.actionSet( gameState.getLocationID(agentIndex) );
	...
	for( int o = 0; o < GameAction.NUM_ACTIONS; o++ ) {
		 ...
		 * use drays package
		 */
		SizableProblemI problem = new Problem(13, 4);
